...
public:
    ...
    // Every cell in this matrix is filled from left to right for each row.
    // This essentially turns the SparseMatrix into a dense one.
    void setAll()
    {
        mRowIndices.resize(mRows * mCols);
        mColIndices.resize(mRows * mCols);
    ...
    // Sets a random percentage of the cells in this matrix.
    void setRandom(Rand& rand, const double fillPercentage)
    {
        size_t numConnections = (size_t)(fillPercentage * mRows * mCols);
    ...
    {
        for (size_t i = 0; i < mRowIndices.size(); ++i)
        {
    ...
    {
        const size_t* rowIndices = mRowIndices.data();
        const size_t* colIndices = mColIndices.data();
        const size_t N           = mRowIndices.size();

        // Calculate y = mData * x
        std::fill(y, y + mRows, T{});
        for (size_t i = 0; i < N; ++i)
        {
            y[rowIndices[i]] += mData[i] * x[colIndices[i]];
        }
    }
    ...
    void multiplyTranspose(const T* x, T* y) const
    {
        ...
        for (size_t i = 0; i < N; ++i)
        {
            y[colIndices[i]] += mData[i] * x[rowIndices[i]];
        }
    }
    ...
    void outerProduct(const T* x, const T* y, T* A) const
    {
        ...
        for (size_t i = 0; i < N; ++i)
        {
            A[i] += x[rowIndices[i]] * y[colIndices[i]];
        }
    }
    ...
    Matrix<T> toMatrix() const
    {
        ...
        for (size_t i = 0; i < N; ++i)
            res(rowIndices[i], colIndices[i]) = mData[i];
    ...
    size_t getNumConnections() const { return mRowIndices.size(); }

private:
    T* mData;
    size_t mRows, mCols;
    vector<size_t> mRowIndices;
    vector<size_t> mColIndices;
...
//         mRowIndices(new size_t[numConnections]),
